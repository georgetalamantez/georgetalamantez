...
async function getTitleFromDOI(doi) {
    doi = doi.replace('@', '/'); // Convert '@' to '/' for proper DOI format
    try {
        const response = await fetch(`${crossrefUrl}${doi}`);
        if (response.ok) {
            const data = await response.json();
            return data.message.title[0];
        } else {
            console.error(`Failed to retrieve title for DOI ${doi}: Status ${response.status}`);
        }
    } catch (error) {
        console.error(`Error retrieving title for DOI ${doi}: ${error}`);
    }
...
async function uploadFile(filePath, cookie) {
    ...
    const doi = path.parse(filename).name; // Extract DOI from filename
    const title = await getTitleFromDOI(doi); // Fetch title from DOI

    const formData = new FormData();
    formData.append('file', fs.createReadStream(filePath));
    formData.append('tags', '');
    formData.append('describe', title); // Include title in description
