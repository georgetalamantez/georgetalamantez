 ...
 */
function findUrls($, baseUrl, keyword, extensions = []) {
  const links = [];
  $('a[href]').each((_, el) => {
    const href = $(el).attr('href');
    if (href && href.includes(keyword) && (extensions.length === 0 || extensions.some(ext => href.endsWith(ext)))) {
...
async function crawlIpfs(startUrl, visited, results) {
  ...
  while (toVisit.length > 0) {
    ...
    // Collect URLs with specified file extensions
    const matchingLinks = findUrls($, currentUrl, '?filename=', FILE_EXTENSIONS);
    ...
    // Find and queue pages with ?filename= (without filtering by file extensions)
    const subPages = findUrls($, currentUrl, '?filename=').filter(url => 
