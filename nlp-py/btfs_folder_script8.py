...
def upload_all_files_in_folder(folder_path, cookie):
    ...
    # Get list of all files in the folder and sort by file size
    files_list = [
        os.path.join(folder_path, f) for f in os.listdir(folder_path)
        if os.path.isfile(os.path.join(folder_path, f))
    ]
    files_list.sort(key=os.path.getsize)  # Sort files by ascending file size
    ...
    # Proceed with normal upload process for remaining files in ascending file size order
