import aiohttp
import asyncio
import os
import re

# Path to the request session file
session_file = "c:/users/owner/downloads/request_session.txt"
log_file = "c:/users/owner/downloads/upload_log.txt"  # Log file to track successful uploads
failed_log_file = "c:/users/owner/downloads/failed_uploads.txt"  # Log file to track failed uploads
response_log_file = "c:/users/owner/downloads/api_response_log.txt"  # Log file for API responses

# Extract the cookie from the session file
def extract_cookie(session_file):
    with open(session_file, 'r') as file:
        data = file.read()
        cookie_match = re.search(r'Cookie: (.*)', data)
        if cookie_match:
            return cookie_match.group(1)
        return None

# Save the successfully uploaded file name to the log
def log_uploaded_file(file_path):
    with open(log_file, 'a') as log:
        log.write(f"{file_path}\n")

# Save the failed upload file name to the failed log
def log_failed_upload(file_path):
    with open(failed_log_file, 'a') as log:
        log.write(f"{file_path}\n")

# Remove a successfully uploaded file from the failed log
def remove_from_failed_log(file_path):
    if os.path.exists(failed_log_file):
        with open(failed_log_file, 'r') as log:
            lines = log.readlines()
        with open(failed_log_file, 'w') as log:
            for line in lines:
                if line.strip() != file_path:
                    log.write(line)

# Load the list of uploaded files from the log
def load_uploaded_files():
    if os.path.exists(log_file):
        with open(log_file, 'r') as log:
            return set(log.read().splitlines())
    return set()

# Load the list of failed uploads from the failed log
def load_failed_uploads():
    if os.path.exists(failed_log_file):
        with open(failed_log_file, 'r') as log:
            return set(log.read().splitlines())
    return set()

# Log API response (both success and failure) along with the file path
def log_api_response(file_path, status_code, response_body):
    with open(response_log_file, 'a') as log:
        log.write(f"File: {file_path}, Status: {status_code}, Response: {response_body}\n")

# Asynchronously upload a file with retry logic
async def upload_file(file_path, cookie, file_number, total_files, max_retries=2):
    url = "https://finder.btfs.io/api/v1/gateway/upload_file"
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/129.0.0.0 Safari/537.36',
        'Cookie': cookie
    }

    for attempt in range(max_retries + 1):  # Attempt retries
        try:
            async with aiohttp.ClientSession() as session:
                with open(file_path, 'rb') as file_to_upload:
                    files = {'file': file_to_upload}
                    print(f"Uploading file {file_number} of {total_files}: {file_path} (Attempt {attempt + 1})")
                    async with session.post(url, headers=headers, data=files) as response:
                        status_code = response.status
                        response_body = await response.text()  # Capture the response body
                        log_api_response(file_path, status_code, response_body)  # Log the API response

                        if status_code == 200:
                            print(f"File '{file_path}' uploaded successfully!")
                            return True  # Successful upload
                        elif status_code == 504:
                            print(f"Gateway Timeout (504) encountered. Retrying {attempt + 1}/{max_retries}...")
                            await asyncio.sleep(2)  # Wait before retry
                        else:
                            print(f"Failed to upload file '{file_path}'. Status code: {status_code}")
                            return False
        except Exception as e:
            print(f"An error occurred: {e}")
            await asyncio.sleep(2)  # Brief pause before retry

    print(f"Max retries reached for '{file_path}'. Marking as failed.")
    return False  # Failed upload after retries

# Asynchronously upload all files in a folder, retry failed uploads first
async def upload_all_files_in_folder(folder_path, cookie):
    uploaded_files = load_uploaded_files()  # Load the uploaded files log
    failed_files = load_failed_uploads()  # Load the failed files log
    files_list = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if os.path.isfile(os.path.join(folder_path, f))]
    total_files = len(files_list)

    # Retry failed uploads first
    for failed_file in failed_files:
        if os.path.exists(failed_file) and await upload_file(failed_file, cookie, 0, total_files):
            log_uploaded_file(failed_file)
            remove_from_failed_log(failed_file)

    # Proceed with normal upload process for remaining files
    tasks = []
    for i, file_path in enumerate(files_list, start=1):
        if file_path in uploaded_files:
            print(f"File '{file_path}' already uploaded. Skipping.")
            continue

        tasks.append(upload_file_with_logging(file_path, cookie, i, total_files))

    await asyncio.gather(*tasks)

# Wrapper to handle logging for each file upload
async def upload_file_with_logging(file_path, cookie, file_number, total_files):
    if await upload_file(file_path, cookie, file_number, total_files):
        log_uploaded_file(file_path)  # Log on success
    else:
        log_failed_upload(file_path)  # Log on failure

# Extract the cookie and start the upload process
cookie = extract_cookie(session_file)
if cookie:
    folder_path = "c:/users/owner/downloads/listing/libgen"
    asyncio.run(upload_all_files_in_folder(folder_path, cookie))
else:
    print("Cookie not found in the session file.")
